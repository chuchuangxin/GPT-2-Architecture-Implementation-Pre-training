...
class FeedForward(nn.Module):
    # 所有 PyTorch 的 nn.Module 子类都需要在 __init__ 里定义层结构。
    def __init__(self):
        # Super 继承 确保 FeedForward 继承 nn.Module 的功能（如神经网络定义等等）
        super().__init__()
        self.d_model = d_model
        self.dropout = dropout
        # nn.Sequential
        # self 代表的是当前对象的实例。在 FeedForward 这个类里：self.ffn 是 FeedForward 这个实例的一个属性。
    ...
    def forward(self, x):
        # 等价于self.ffn.forward(x)         nn.Sequential 继承了 nn.Module
        return self.ffn(x)


# Define Scaled Dot Product Attention
# init为构造函数
# head_size传参时候确立 可以自由更改
# 而其余的值直接在创建对象时候 直接使用全局变量传递
